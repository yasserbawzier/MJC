...
function handleExcelFileSelected(event) {
    ...
    if (typeof XLSX === 'undefined') {
        InvoiceItemsUI.showToast('❌ مكتبة قراءة Excel غير محملة! تأكد من اتصال الإنترنت.', 'error');
        return;
    }
    ...
    reader.onload = (e) => {
        try {
            ...
            if (!allRows || allRows.length === 0) {
                InvoiceItemsUI.showToast('⚠️ الملف فارغ أو لا يحتوي على بيانات!', 'error');
                return;
            }
            ...
            if (dataRows.length === 0) {
                InvoiceItemsUI.showToast('⚠️ لا توجد بيانات أسفل صف العناوين!', 'error');
                return;
            }
            ...
            if (Object.keys(headerMapping).length === 0) {
                InvoiceItemsUI.showToast('❌ لم يتم التعرف على أي عمود! افتح Console (F12) لرؤية التفاصيل.', 'error');
                return;
            }
                ...
                if (localStartRow !== undefined) {
                    ...
                    if (rowspan > 1 || colspan > 1) {
                        ...
                        if (dbField) {
                            ...
                            // نسخ قيمة الخلية العلوية (المدمجة) إلى باقي الخلايا السفلية لكي لا تتعطل الحسابات
                            if (rowspan > 1) {
                                const primaryValue = excelParsedRows[localStartRow][dbField];
                                for (let r = localStartRow + 1; r <= localEndRow; r++) {
                                    if (excelParsedRows[r]) {
                                        excelParsedRows[r][dbField] = primaryValue;
                                    }
                                }
                            }
                        }
            ...
            if (excelParsedRows.length === 0) {
                InvoiceItemsUI.showToast('⚠️ لم يُعثر على بيانات صالحة! تأكد من أن الملف يحتوي على بيانات أسفل العناوين.', 'error');
                return;
            }
            ...
            InvoiceItemsUI.showToast('❌ خطأ في قراءة الملف: ' + err.message, 'error');
        }
    };
...
async function confirmExcelImport() {
    if (!State.currentInvoiceId) {
        InvoiceItemsUI.showToast('⚠️ لا يوجد معرف فاتورة نشط!', 'error');
        return;
    }
    if (!excelParsedRows || excelParsedRows.length === 0) {
        InvoiceItemsUI.showToast('⚠️ لا توجد بيانات للاستيراد!', 'error');
        return;
    }
    ...
    const toast = InvoiceItemsUI.showToast('جاري استيراد ' + excelParsedRows.length + ' صف...', 'info');

    try {
        // حساب بداية الترقيم من آخر عنصر موجود
        const startIndex = State.invoiceItems.length + 1;
        ...
        newItems.forEach(item => {
            ...
            InvoiceItemsController.recalculateLocalItemFields(item, false);
            // تهيئة لوك أب الصور
            State.itemPhotosLookup[item.id] = {
        ...
        InvoiceItemsController.renderTable();
