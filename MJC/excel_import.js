...
function handleExcelFileSelected(event) {
    ...
    reader.onload = (e) => {
        try {
            ...
            const dataRows  = allRows.slice(bestHeaderRowIndex + 1);
            ...
            // مع دعم الخلايا المدمجة عمودياً (forward-fill)
            const lastValues = {}; // لتعبئة الخلايا المدمجة

            excelParsedRows = dataRows.map(row => {
                const mapped = {};
                Object.entries(headerMapping).forEach(([colIdxStr, dbField]) => {
                    const colIdx = parseInt(colIdxStr);
                    let rawVal   = row[colIdx];

                    // forward-fill: إذا كانت الخلية فارغة واحتمال أنها مدمجة
                    // نستخدم آخر قيمة لنفس العمود فقط للحقول النصية
                    if ((rawVal === '' || rawVal === undefined || rawVal === null) && !NUMERIC_DB_FIELDS.has(dbField)) {
                        rawVal = lastValues[colIdx] || '';
                    }

                    if (rawVal !== '' && rawVal !== undefined && rawVal !== null) {
                        lastValues[colIdx] = rawVal;
                    }
